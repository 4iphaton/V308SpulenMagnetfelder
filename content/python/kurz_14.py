...
xi, Bi = np.genfromtxt('content/values/Spulek_1,4i.txt',unpack=True)
xi = xi - 3
xi = xi / 100
Bi = Bi / 1000
xal, Bal = np.genfromtxt('content/values/Spulek_1,4al.txt',unpack=True)
xal = xal - 8
xal = xal / 100
Bal = Bal / 1000
xar, Bar = np.genfromtxt('content/values/Spulek_1,4ar.txt',unpack=True)
xar = xar + 4
xar = xar / 100
Bar = Bar / 1000
Nl = 100
...
ln4= np.linspace(-0.08, 0.09, 5000)
#ln2 = np.linspace(xa[0],xa[len(xa)-1],5000)

#plt.plot(ln2, fa(ln2), 'g-', label='linksseitig außen')#'Theoriekurve linksseitig außen')
#plt.plot(ln3, fa(ln3), 'y-', label='rechtsseitig außen')#'Theoriekurve rechtsseitig außen')
plt.plot(ln4, fa(ln4), 'g-', label='Biot-Savart')
